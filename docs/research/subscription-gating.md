...
- **Provider: for an Argentina-registered seller, a Merchant of Record (MoR) isn't just preferable — it's effectively the only route.** Raw Stripe is unavailable: Argentina is **not** among Stripe's ~46 supported merchant countries ([stripe.com/global](https://stripe.com/global)); the usual workaround is incorporating a US Delaware LLC via Stripe Atlas (~$500 + ongoing US compliance) — a whole second tax life, out of proportion for this product. Meanwhile UK and EU VAT on B2C digital sales are owed **from the first sale, no threshold, for any non-established seller** ([Stripe EU VAT guide](https://stripe.com/guides/introduction-to-eu-vat-and-european-vat-oss), [UK digital-products guide](https://stripe.com/resources/more/uk-vat-on-digital-products)) — obligations an Argentine sole trader cannot realistically carry. An MoR resells the subscription, absorbs all consumer-side tax, and — crucially — both shortlisted MoRs **onboard Argentina-based sellers**: **Paddle** (Argentina on its supported-countries list; 5% + $0.50; payouts worldwide) and **Polar** (Argentina explicitly listed; payouts via Stripe Connect Express, which works even where Stripe Payments doesn't; **repriced 2026-05-27** — free tier now 5% + 50¢, Pro at $20/mo drops to 3.8% + 40¢). On the Argentine side the structure is clean: you export a service to **one foreign company** (the MoR) and invoice it with **Factura E at 0% IVA** — no per-consumer, per-country tax surface at all. Stripe's own MoR (Managed Payments) charges **3.5% on top of** normal Stripe fees (~6.4%+ all-in) *and* would require the foreign Stripe entity anyway — doubly excluded.
...
The fundamental fork: with **Stripe direct** you are the merchant of record — cheapest fees, but *you* owe tax registration/filing wherever your consumers are (and you need a Stripe-supported entity at all). With a **Merchant of Record** the provider legally resells your product — higher fees, zero consumer-side tax surface. For a solo developer tax-registered in Argentina selling a low-priced consumer subscription worldwide, this fork is decided before it starts: see below.

### 2.1 Tax reality for an Argentina-registered seller (why MoR is the only sane answer)

**Consumer-side (where the buyers are):**

- **UK & EU:** for B2C digital services sold by a **non-established** business there is **no registration threshold in either** — UK VAT from the first UK consumer sale, EU VAT from the first EU sale in the consumer's member state at that state's rate ([Stripe — UK VAT on digital products](https://stripe.com/resources/more/uk-vat-on-digital-products), [Stripe — EU VAT & OSS](https://stripe.com/guides/introduction-to-eu-vat-and-european-vat-oss)). The non-Union OSS scheme collapses EU filing to one quarterly return, but it's still registrations, quarterly returns, location-evidence record-keeping, and rate maintenance — from sale #1, in a foreign language of tax law, forever.
- **US:** state-by-state economic nexus for SaaS; irrelevant at first, a real burden later.
- Subscription VAT must be reassessed against **customer location at each renewal**.

Stripe Tax-style tooling **calculates and collects** but does **not** register or file for you. An MoR removes the obligation entirely because you sell to the MoR and the MoR sells to the consumer ([Fungies MoR guide](https://fungies.io/merchant-of-record-complete-guide-2/)).

**Seller-side (Argentina):** with an MoR, what Makrofy does is an **export of services to a single foreign company** (Paddle/Polar), which is the best-understood shape in the Argentine regime:

- Export of services is **outside the scope of IVA (0%)** and is invoiced with a **Factura E** (electronic export invoice) to the foreign counterparty. A **monotributista of any category (A–K) can issue Factura E** without becoming Responsable Inscripto ([ARCA — exportación de servicios en monotributo](https://www.afip.gob.ar/monotributo/exportacion-servicios/), [Factura E guide 2026](https://contablix.ar/blog/factura-e-exportacion-servicios-2026)).
- Practical rhythm: one Factura E per MoR payout (or per month), in USD, to one counterparty — versus thousands of per-consumer invoices in dozens of countries under the raw-Stripe model. This asymmetry alone decides the provider question.
- **Payout logistics:** Paddle pays out worldwide (Payoneer/wire among the rails); Polar pays out via **Stripe Connect Express, which supports Argentina even though Stripe Payments doesn't** — customer payments go to Polar (US) and only the transfer touches Connect ([Polar — supported countries](https://polar.sh/docs/merchant-of-record/supported-countries)). Argentina's capital controls were substantially dismantled in 2025 ([trade.gov note](https://www.trade.gov/market-intelligence/argentina-eliminates-capital-controls-and-payment-timelines)), but how a payout must land (USD account, conversion obligations, timing) is regime-sensitive and moving — **verify the current cobro-del-exterior rules with an accountant at build time**.
- **Flag for the accountant, not this note:** (a) whether export-of-services income counts toward the monotributo category caps under current ARCA rules (coverage claims it doesn't for genuine service exports post-2021 — treat as unverified); (b) the **Mono-Tech** regime (billing and *keeping* dollars for tech exporters) had only media sanción in Diputados and was **not law as of mid-2026** ([Argentina.gob.ar](https://www.argentina.gob.ar/noticias/diputados-da-media-sancion-al-proyecto-de-ley-que-permite-freelancers-exportadores-de)) — track it, don't plan on it; (c) provincial **Ingresos Brutos** treatment of exported services in your province; (d) sales to **Argentine** consumers are *not* exports — they're domestic B2C, which the MoR relationship doesn't change for your own tax position (the MoR still handles the consumer side; your invoice to the MoR stays an export — but confirm the characterisation).

**Verdict:** MoR, with no realistic alternative. Raw Stripe would require a foreign entity *plus* worldwide consumer-tax admin; the MoR route reduces Argentina-side compliance to routine export invoicing a monotributista already knows how to do.

### 2.2 Stripe direct (for completeness / the cost floor)

- **Availability blocker:** **Argentina is not a Stripe-supported merchant country** ([stripe.com/global](https://stripe.com/global) — ~46 countries, none of them Argentina). The standard workaround for LATAM founders is a **US Delaware LLC via Stripe Atlas** ($500 setup plus annual US franchise tax, registered agent, US filings, and a US bank relationship) — a permanent second compliance regime adopted solely to reach a payment processor. Everything below assumes that entity exists; without it this whole option is moot.
- **Fees (illustrative, UK-account rates from [stripe.com/gb/pricing](https://stripe.com/gb/pricing); US-entity rates differ in detail, not in shape):** domestic standard cards **1.5% + 20p** (premium 1.9% + 20p), EEA 2.5% + 20p, international 3.25% + 20p, +2% currency conversion; **Billing 0.7%** of recurring volume; **Tax Basic 0.5%**/transaction. All-in on a domestic card: ~**2.7% + 20p**.
...
- **Verdict:** cheapest fees, best docs, most control — but for an Argentina-registered seller it costs a foreign company *plus* the full worldwide consumer-tax burden. Revisit only if MoR fees ever become material at scale (a good problem; subscription migration out of an MoR is possible but painful).
...
|---|---|---|---|---|
| Fee | 5% + $0.50 | **5% + 50¢** free tier since the **2026-05-27 repricing** (was 4% + 40¢; grandfathered for pre-existing orgs); **3.8% + 40¢ on the $20/mo Pro plan** ([Dodo review](https://dodopayments.com/blogs/polar-sh-review)) | 5% + $0.50 | 3.5% **on top of** Stripe fees → ~6.4% + 30¢ all-in, worst-case >8–10% intl. |
| **Argentina-based seller** | ✅ On the [supported-countries list](https://www.paddle.com/help/start/intro-to-paddle/which-countries-are-supported-by-paddle); payouts worldwide (Payoneer/wire) | ✅ Explicitly listed ([supported countries](https://polar.sh/docs/merchant-of-record/supported-countries)); payouts via Stripe **Connect Express**, which covers Argentina even though Stripe Payments doesn't; individuals OK where Connect supports the "individual" business type | — | ❌ Requires a Stripe-supported entity |
...
| Maturity/risk | Established, at-scale SaaS standard | Younger (10k devs early 2026), open-source, dev-first; just repriced once — expect further pricing motion | **Sunsetting** — being migrated into Stripe Managed Payments after acquisition | Public preview (Feb 2026), ~35 countries |
...
| Route | Fees ≈ | Note |
|---|---|---|
| Stripe direct (domestic card, +Billing+Tax) | **~£1.00** | **not available** without first running a US/UK entity; plus worldwide VAT admin |
| Paddle | **~£1.87** | zero tax admin; Factura E to Paddle |
| Polar (free tier, post-repricing) | **~£1.90** | zero tax admin; Factura E to Polar |
| Polar (Pro, $20/mo) | ~£1.45 + $240/yr flat | only worth it above ~$1.5–2k/mo revenue |
| Stripe Managed Payments | ~£2.40+ | needs a Stripe entity anyway — excluded |

The absolute differences are noise at consumer price points; choose on **risk and DX**, not fee — Polar's May 2026 repricing erased its fee edge at the free tier, so the two shortlisted options now cost the same.

**Verdict:** **Paddle if you want the boring, proven choice; Polar if you value DX and accept a younger vendor that has already repriced once.** Fee parity means the tiebreakers are Paddle's maturity/tax-coverage depth vs. Polar's integration speed. Both onboard Argentina-based sellers, and both need the same Makrofy-side build: one webhook Cloud Function + an entitlement doc. Avoid Lemon Squeezy (explicitly in migration) and Stripe Managed Payments (fee stacking + entity requirement) for a new build.
...
3. Paddle vs Polar (vendor-risk appetite; fees are now equal at Polar's free tier — the tiebreak is maturity vs DX).
3a. **Accountant session (Argentina):** Factura E cadence per MoR payout; whether export income counts toward monotributo category caps under current ARCA rules; Ingresos Brutos on exported services in your province; how MoR payouts should land (USD/conversion) under the post-2025 FX regime; Mono-Tech status.

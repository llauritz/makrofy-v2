...
- **`flagged` persists past commit** even though V2's day list doesn't render it — kept so a future edit form or "review uncertain entries" feature has the data. The AI's interpretation text and grounding state are deliberately *not* stored; Google attribution is satisfied at response time in the add card.

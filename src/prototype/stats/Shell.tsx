...
// PROTOTYPE — issue #7. The locked "Warm Market" shell (issue #4) reused as the
// host surface: the shell itself is NOT under evaluation, only where stats live
// and which stats appear. Adapted from the add-flow prototype's Shell (issue #5):
// static add card, plus `aboveSummary` / `statsButton` / `onStats` hooks so each
// variant can place its stats surface.
import { BarChart3, CloudOff, Plus, Settings2 } from "lucide-react"

import { GOAL_KCAL } from "./data"
...
// Static add card — visual furniture only; the add flow was settled in issue #5.
function AddCard() {
  return (
...
export function Shell({
  ...
  const totals = SEED_ENTRIES.reduce(
    (acc, e) => ({
